...
let
  ...
  eza        = getExe pkgs.eza;
  microfetch = getExe pkgs.microfetch;
  trashPut   = getExe' pkgs.trash-cli "trash-put";
  trashRest  = getExe' pkgs.trash-cli "trash-restore";
in
{
  programs.fish = {
    enable = true;

    # Simple aliases (HM turns these into fish aliases)
    shellAliases = {
      l    = "${eza} -lh --icons=auto";
      ls   = "${eza} -1 --icons=auto";
    ...
    interactiveShellInit = ''
      ...
      # Functions/aliases that need Fish syntax (kept as-is)
  ...
  # Let HM do the right thing for these:
  programs.direnv = {
    enable = true;
  ...
  # Make sure tools used by your aliases are present
  home.packages = with pkgs; [
    eza trash-cli microfetch figlet lolcat fzf tmux lf
  ];
}
